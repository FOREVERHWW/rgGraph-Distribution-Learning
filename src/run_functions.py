...
def run_model(model_data: geo.data.Data, hidden_channels=64, num_layers=3, lr=.001, weight_decay=5e-4, num_epochs=100,
              display=True, auto_stop=False, conv_crit=1e-4, patience=10, max_epoch=500, save_model=False, data_name="dblp"):
    """
    ...
    :param save_model: if you want to save the model or not
        ...
        for epoch in range(1, num_epochs + 1):
            ...
            if display:
                ...
                print(
                    ...
                    f', Intersection Distance: {int_eval:.4f}, KL Divergence Distance: {kl_eval:.4f}')
    if save_model:
        torch.save(model.state_dict(), f"saved_{data_name}_model")
